...
export default function QuizPage({ id, setValue }) {
  ...
  const [timeLimit, setTimeLimit] = useState(180); // 3 minutes in seconds
  const [showWarning, setShowWarning] = useState(false);
  const [warningDisplayed, setWarningDisplayed] = useState(false);

  const handleWarning = () => {
    window.alert("Warning: Only 1 minute left!");
  ...
  return viewresults ? (
    ...
    <>
      <Timer timeLimit={timeLimit} onTimeUp={onSubmit} />
      {/* {showWarning && (
